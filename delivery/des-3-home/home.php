		...
		<section class="newspaper-lead">
			<div class="container">
				<article class="newspaper-lead-inner">
					...
					<?php else : ?>
						<a class="newspaper-lead-image newspaper-lead-image-fallback" href="<?php echo esc_url( get_permalink() ); ?>">
							<span><?php esc_html_e( 'Featured Game Story', 'h5game' ); ?></span>
						</a>
					<?php endif; ?>
				</article>
			</div>
			...
			if ( $game_query->have_posts() ) :
				...
				<div class="des3-game-grid">
					...
					while ( $game_query->have_posts() ) :
						...
						$game_rating       = 35 + ( absint( get_the_ID() ) % 16 );
						$game_rating_label = number_format_i18n( $game_rating / 10, 1 );
						$game_rating_width = min( 100, max( 0, ( $game_rating / 50 ) * 100 ) );
						?>
						<article class="des3-game-card<?php echo has_post_thumbnail() ? ' has-thumb' : ''; ?>">
							<a href="<?php echo esc_url( get_permalink() ); ?>">
								...
								<?php else : ?>
									<div class="des3-game-media des3-game-media-fallback" aria-hidden="true">
										<span><?php echo esc_html( strtoupper( substr( wp_strip_all_tags( get_the_title() ), 0, 1 ) ) ); ?></span>
									</div>
								<?php endif; ?>
								<strong><?php echo esc_html( get_the_title() ); ?></strong>
								<div class="des3-game-rating" aria-label="<?php echo esc_attr( sprintf( __( 'Rating %s out of 5', 'h5game' ), $game_rating_label ) ); ?>">
									<span class="des3-game-stars" aria-hidden="true"><span style="width: <?php echo esc_attr( $game_rating_width ); ?>%;"></span></span>
									<span><?php echo esc_html( $game_rating_label ); ?></span>
								</div>
							</a>
						</article>
						<?php
