...
$des3_nav_title_filter = static function ( $items ) {
				...
				<?php if ( $has_primary_menu ) : ?>
					<div class="head-nav-row">
						<nav id="primary-menu" class="main-navigation head-menu" aria-label="<?php esc_attr_e( 'Primary Menu', 'h5game' ); ?>">
							<?php
							add_filter( 'wp_nav_menu_objects', $des3_nav_title_filter );
							wp_nav_menu(
								array(
							...
							remove_filter( 'wp_nav_menu_objects', $des3_nav_title_filter );
							?>
						</nav>
					</div>
				<?php endif; ?>
