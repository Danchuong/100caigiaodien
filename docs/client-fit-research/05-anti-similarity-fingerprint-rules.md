...
- `docs/100-home-styles.md`: mỗi design có fingerprint gồm `Header + Hero + Flow/Grid + Card Pattern + Footer + Mobile Signature`.
...
Quy tắc hiện tại trong `docs/100-home-styles.md` là khác ít nhất 4/6 phần lớn. Với 100 bản và nhiều agent cùng làm, nên dùng ngưỡng chặt hơn:
...
Dòng này phải cụ thể hơn dòng trong `docs/100-home-styles.md`. Ví dụ không chỉ ghi `Review Score Hero`, mà ghi:
...
- `des-(N-1)` nếu đã có.
- Mọi bản cùng nhóm trong `docs/100-home-styles.md`.
...
Trước khi chuyển trạng thái sang `Done`:
