...
Áp dụng cho mỗi style `des-N` trước khi đổi trạng thái trong `docs/design-research/100-design-board.md` sang `QA Passed`, `Delivery Ready`, `Shipped` hoặc `Accepted`, copy vào `delivery/des-N-home`, hoặc commit.

## Nguyên tắc pass/fail

Một style bị giữ lại nếu có một trong các lỗi sau:

- Không khác rõ fingerprint trong `docs/design-research/100-design-board.md`; nếu bỏ màu/ảnh/font ra mà wireframe vẫn giống style gần nhất thì chưa đạt.
...
1. **Xác nhận brief của style**
   - Mở block `des-N` trong `docs/design-research/100-design-board.md`.
...
```
   ...
   - Không được có sửa ngoài `wp-content/themes/des-N/`, `delivery/des-N-home/`, `delivery/des-N-home.zip`, và dòng trạng thái trong `docs/design-research/100-design-board.md` nếu người phụ trách style được phép cập nhật.
...
- Không đổi `yeucau.md`.
- Chỉ đổi `docs/design-research/100-design-board.md` khi style đã pass toàn bộ QA và người phụ trách được phép chuyển trạng thái.
