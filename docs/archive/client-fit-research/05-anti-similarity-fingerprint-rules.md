...
- `100-design-board.md`: mỗi design có fingerprint gồm `Header + Hero + Flow/Grid + Card Pattern + Footer + Mobile Signature`.
...
Quy tắc hiện tại trong `100-home-styles.md` là khác ít nhất 4/6 phần lớn, còn spec từng bản nằm trong `100-design-board.md`. Với 100 bản và nhiều agent cùng làm, nên dùng ngưỡng chặt hơn:
...
Dòng này phải cụ thể hơn dòng trong `100-design-board.md`. Ví dụ không chỉ ghi `Review Score Hero`, mà ghi:
...
- `des-(N-1)` nếu đã có.
- Mọi bản cùng nhóm trong `100-design-board.md`.
